...
export function ConfidenceCheck({ sessionId, slots, isFirstSession, onDismiss }: ConfidenceCheckProps) {
  ...
  if (submitted) {
    return (
      ...
      >
        <div className="w-full max-w-md bg-white border border-gray-200 rounded-2xl shadow-lg p-6 text-center">
  ...
  return (
    ...
    >
      <div
        ref={cardRef}
        className="w-full max-w-md bg-white border border-gray-200 rounded-2xl shadow-lg p-5 space-y-4 max-h-[40vh] overflow-y-auto"
      >
        {/* Confidence scale */}
        <div>
          <p className="text-sm font-semibold text-ink mb-2.5">How confident are you feeling?</p>
              ...
              <button
                ...
                className={`w-7 h-7 rounded-lg text-xs font-semibold transition-all ${
                  confidence === n
                    ...
                    : "bg-gray-100 text-ink-muted hover:bg-primary-50 hover:text-primary-600"
                }`}
              >
                {n}
              </button>
        ...
        {unlockedSlots.length > 0 && (
          <div>
            ...
            <select
              ...
              className="w-full text-sm border border-[#DBEAFE] rounded-xl px-3 py-2 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-transparent bg-[#FAFCFF] text-ink"
            >
        ...
        <div>
          ...
          <input
            ...
            className="w-full text-sm border border-[#DBEAFE] rounded-xl px-3 py-2 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-transparent bg-[#FAFCFF] placeholder:text-ink-muted"
          />
        </div>
        ...
        <button
          ...
          disabled={!confidence}
          className="w-full py-2.5 bg-primary-500 hover:bg-primary-600 text-white font-semibold text-sm rounded-xl transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
        >
          Save &amp; Good Luck
        </button>
      </div>
