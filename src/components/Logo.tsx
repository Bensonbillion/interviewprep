...
function SMonogram({ size, theme }: { size: number; theme: "light" | "dark" | "auto" }) {
  if (theme === "auto") {
    return (
      <svg
        width={size}
        ...
        aria-hidden="true"
      >
        <rect width="48" height="48" rx="12" className="fill-stone-900 dark:fill-cream-50" />
        <text
          x="12"
          y="36"
          fontFamily="'DM Serif Display', serif"
          fontSize="34"
          className="fill-cream-50 dark:fill-stone-900"
        >
          S
        </text>
...
export function LogoIcon({ size = "md", theme = "auto" }: LogoProps) {
  return <SMonogram size={SIZES[size].icon} theme={theme} />;
}

export function LogoWordmark({ size = "md", theme = "auto" }: LogoProps) {
  const color = theme === "auto"
    ? "text-stone-900 dark:text-[var(--text-primary)]"
    : theme === "light" ? "text-[#1A1A1A]" : "text-[#FDFCFA]";
  return (
    <span className={`${SIZES[size].text} ${color} inline-flex items-baseline`}>
      <span className="font-serif">SalesPrep</span>
      <span className="font-sans font-medium text-coral ml-1">AI</span>
    </span>
  );
}

export function LogoFull({ size = "md", theme = "auto" }: LogoProps) {
  return (
