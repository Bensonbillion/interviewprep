...
-- License: BSD-2-Clause (see LICENSE.md in upstream repo)
--
-- This seed runs after migrations during `supabase start` and `supabase db
-- reset`; pgTAP test files in supabase/tests/ rely on the tests.* helpers
-- created here. seed.sql is NOT pushed to prod by `supabase db push`.
--
-- The original file's first two lines (\echo ... \quit) were stripped — they
-- exist so the file refuses to load via direct psql when intended for
-- CREATE EXTENSION. We're loading via seed.sql, so we want it to run.
-- ─────────────────────────────────────────────────────────────────────────────
